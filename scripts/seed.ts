  ...
  data: [
    ...
    await createProduct(
      ...
      [
        ...
        productImages.moskyAudio_overdrive_dolphin_photo_3,
      ]
    );
    ...
    // Create MoskyAudio Album
    const moskyAudioAlbum = await createAlbum(
      'mosky-audio-album-001',
      'MoskyAudio Product Gallery',
      'Colección completa de productos MoskyAudio incluyendo pedales de efectos, fuentes de alimentación y accesorios de alta calidad para guitarristas.',
      productImages.moskyAudio_g_iii_blue_purple_photo_0
    );

    // Create M-VAVE Album
    const mvaveAlbum = await createAlbum(
      'mvave-album-001',
      'M-VAVE Product Gallery',
      'Galería de productos M-VAVE featuring pedales de efectos, sistemas inalámbricos y equipos de audio profesional.',
      productImages.mvave_mini_universe_photo_0
    );

    console.log(' Albums created');

    // Add product photos to albums
    console.log('Adding photos to albums...');

    // Add MoskyAudio products to MoskyAudio album
    let sortOrder = 0;
    
    await addAlbumPhoto(moskyAudioAlbum.id, 'g-iii-blue-purple-001', 'MoskyAudio G III BLUE Purple Channel', sortOrder++);
    await addAlbumPhoto(moskyAudioAlbum.id, 'overdrive-dolphin-001', 'MoskyAudio DOLPHIN OverDrive', sortOrder++);
    await addAlbumPhoto(moskyAudioAlbum.id, 'lm741-preamp-001', 'MoskyAudio LM741 PREAMP', sortOrder++);
    await addAlbumPhoto(moskyAudioAlbum.id, 'cosmic-blue-001', 'MoskyAudio COSMIC BLUE', sortOrder++);
    await addAlbumPhoto(moskyAudioAlbum.id, 'sky-reverb-delay-001', 'MoskyAudio SKY REVERB & DELAY', sortOrder++);
    await addAlbumPhoto(moskyAudioAlbum.id, 'sol918-multi-effects-001', 'MoskyAudio SOL918 Multi-Effects', sortOrder++);
    await addAlbumPhoto(moskyAudioAlbum.id, 'blue-ocean-chorus-001', 'MoskyAudio Blue Ocean Chorus', sortOrder++);
    await addAlbumPhoto(moskyAudioAlbum.id, 'mad-cow-001', 'MoskyAudio MAD COW', sortOrder++);
    await addAlbumPhoto(moskyAudioAlbum.id, 'iso10-power-supply-001', 'MoskyAudio ISO-10 POWER SUPPLY', sortOrder++);
    await addAlbumPhoto(moskyAudioAlbum.id, 'moskyaudio-effect-pedal-cable-001', 'MoskyAudio Effect Pedal Cable', sortOrder++);

    // Add M-VAVE products to M-VAVE album
    sortOrder = 0;
    
    await addAlbumPhoto(mvaveAlbum.id, 'mvave-mini-universe-001', 'M-VAVE Mini Universe', sortOrder++);
    await addAlbumPhoto(mvaveAlbum.id, 'mvave-elemental-001', 'M-VAVE Elemental', sortOrder++);
    await addAlbumPhoto(mvaveAlbum.id, 'mvave-annblackbox-001', 'M-VAVE Ann Black Box', sortOrder++);
    await addAlbumPhoto(mvaveAlbum.id, 'mvave-baby-001', 'M-VAVE Baby', sortOrder++);
    await addAlbumPhoto(mvaveAlbum.id, 'mvave-chocolate-001', 'M-VAVE Chocolate', sortOrder++);
    await addAlbumPhoto(mvaveAlbum.id, 'mvave-ir-box-001', 'M-VAVE IR Box', sortOrder++);
    await addAlbumPhoto(mvaveAlbum.id, 'mvave-tank-g-001', 'M-VAVE Tank G', sortOrder++);
    await addAlbumPhoto(mvaveAlbum.id, 'mvave-tank-b-001', 'M-VAVE Tank B', sortOrder++);
    await addAlbumPhoto(mvaveAlbum.id, 'mvave-pedal-power-001', 'M-VAVE Pedal Power', sortOrder++);
    await addAlbumPhoto(mvaveAlbum.id, 'mvave-wp-9-001', 'M-VAVE WP-9', sortOrder++);
    await addAlbumPhoto(mvaveAlbum.id, 'mvave-wireless-in-ear-monitor-001', 'M-VAVE Wireless In-Ear Monitor', sortOrder++);
    await addAlbumPhoto(mvaveAlbum.id, 'mvave-wp-8-wireless-001', 'M-VAVE WP-8 Wireless', sortOrder++);

    console.log(' Photos added to albums');
